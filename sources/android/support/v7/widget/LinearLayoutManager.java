...
public class LinearLayoutManager extends LayoutManager implements ScrollVectorProvider, ViewDropHandler {
            ...
            if (supportsPredictiveItemAnimations()) {
                ...
                for (int i5 = 0; i5 < size; i5++) {
                    ...
                    if (!viewHolder.isRemoved()) {
                        ...
                        if (i6 == -1) {
                            i3 += linearLayoutManager.mOrientationHelper.getDecoratedMeasurement(viewHolder.itemView);
                        } else {
                ...
                if (i3 > 0) {
                    ...
                    linearLayoutManager.mLayoutState.mExtra = i3;
    ...
    public void collectAdjacentPrefetchPositions(int i, int i2, State state, LayoutPrefetchRegistry layoutPrefetchRegistry) {
        ...
        if (getChildCount() != 0) {
            if (i != 0) {
                ensureLayoutState();
                updateLayoutState(i > 0 ? 1 : -1, Math.abs(i), 1, state);
    ...
    View findOnePartiallyOrCompletelyInvisibleChild(int i, int i2) {
        ...
        if (this.mOrientation == 0) {
            i = this.mHorizontalBoundCheck.findOneViewWithinBoundFlags(i, i2, i3, i4);
        } else {
            i = this.mVerticalBoundCheck.findOneViewWithinBoundFlags(i, i2, i3, i4);
        }
        return i;
    }
    ...
    public View onFocusSearchFailed(View view, int i, Recycler recycler, State state) {
        ...
        ensureLayoutState();
        ensureLayoutState();
        updateLayoutState(view, (int) (((float) this.mOrientationHelper.getTotalSpace()) * MAX_SCROLL_FACTOR), false, state);
        this.mLayoutState.mScrollingOffset = Integer.MIN_VALUE;
        this.mLayoutState.mRecycle = false;
