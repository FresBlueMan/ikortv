...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser xmlPullParser, Resources resources) {
        TypedArray obtainAttributes = resources.obtainAttributes(Xml.asAttributeSet(xmlPullParser), C0194R.styleable.FontFamily);
        String string = obtainAttributes.getString(C0194R.styleable.FontFamily_fontProviderAuthority);
        String string2 = obtainAttributes.getString(C0194R.styleable.FontFamily_fontProviderPackage);
        String string3 = obtainAttributes.getString(C0194R.styleable.FontFamily_fontProviderQuery);
        int resourceId = obtainAttributes.getResourceId(C0194R.styleable.FontFamily_fontProviderCerts, 0);
        int integer = obtainAttributes.getInteger(C0194R.styleable.FontFamily_fontProviderFetchStrategy, 1);
        int integer2 = obtainAttributes.getInteger(C0194R.styleable.FontFamily_fontProviderFetchTimeout, DEFAULT_TIMEOUT_MILLIS);
    ...
    private static FontFileResourceEntry readFont(XmlPullParser xmlPullParser, Resources resources) {
        resources = resources.obtainAttributes(Xml.asAttributeSet(xmlPullParser), C0194R.styleable.FontFamilyFont);
        int i = resources.getInt(C0194R.styleable.FontFamilyFont_fontWeight, NORMAL_WEIGHT);
        boolean z = true;
        if (1 != resources.getInt(C0194R.styleable.FontFamilyFont_fontStyle, 0)) {
            z = false;
        }
        int resourceId = resources.getResourceId(C0194R.styleable.FontFamilyFont_font, 0);
        String string = resources.getString(C0194R.styleable.FontFamilyFont_font);
        resources.recycle();
        while (xmlPullParser.next() != 3) {
            skip(xmlPullParser);
        }
