...
final class SwipeProgressBar {
    ...
    void draw(Canvas canvas) {
        ...
        float f2 = ((float) ((currentAnimationTimeMillis - r6.mStartTime) % 2000)) / 20.0f;
        int i3 = 0;
        if (r6.mRunning) {
            f = f2;
        } else {
            f = f2;
            if (currentAnimationTimeMillis - r6.mFinishTime >= 1000) {
                r6.mFinishTime = 0;
                return;
            }
            float f3 = (((float) ((currentAnimationTimeMillis - r6.mFinishTime) % 1000)) / 10.0f) / 100.0f;
            float f4 = (float) i;
            f3 = INTERPOLATOR.getInterpolation(f3) * f4;
            r6.mClipRect.set(f4 - f3, 0.0f, f4 + f3, (float) height);
        ...
        if (r6.mTriggerPercentage > 0.0f && r13 != 0) {
            canvas2.restoreToCount(save);
            width = canvas.save();
